...
function Musait(prop) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const response = await axios.get('http://localhost:3006/api/ogrenci/ogrenciMusaitlikGetir', {
        params: { TC_NO: TC_NO },
      });
      setRows(response.data);
    };

    fetchData(); // Sadece ilk yüklemede veya TC_NO değiştiğinde çalışır
  }, [TC_NO]); // TC_NO değiştiğinde tekrar çalıştır
  rows.forEach((element) => {
    MusaitlikProgram[element.DERS_NO][element.GUN] = 'Available';
  });
  ...
  return (
    <>
      {(MusaitlikProgram[DERS_NO - 1][GUN - 1] === 'Available') && (
        <>
          Available
        </>
      )}
      ...
      {student && (
        <div>
          ...
          {isEditing ? (
            ...
            <>
              <p>Name: {student.ISIM}</p>
              <p>Surname: {student.SOYISIM}</p>
              <p>Address: {student.ADRES}</p>
              <p>TC Id: {student.TC_NO}</p>
              <p>Phone: {student.TEL_NO}</p>
              <p>E-mail: {student.E_POSTA}</p>
              <button type="button" onClick={handleEditBtn}>Update Student Information</button>
              {!isDersTalep && (
                <button type="button" onClick={(e) => setisDersTalep(true)}>New Course Request</button>
              )}

              {isDersTalep && (
                <>
                  <button type="button" onClick={(e) => setisDersTalep(false)}>Cancel Course Request</button>
                  <Dersler />
                </>
              )}
            </>
