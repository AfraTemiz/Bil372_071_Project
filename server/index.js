...
app.get("/api/ogrenci/aktifOgrenciSil", (req, res) => {
    const { TC_NO } = req.query;
	const actual_TC_NO = typeof TC_NO === 'object' ? TC_NO.TC_NO : TC_NO;
	console.log("Gelen TC_NO yeni:", TC_NO); // Gelen değeri logla
    if (TC_NO !== undefined) {
        // İşlemi başlat
        db.transaction().then((transaction) => {
            // Önce aktif tablosundan sil
            return db.query(`DELETE FROM aktif WHERE TC_NO = '${actual_TC_NO}'`, { transaction: transaction })
                .then(() => {
                    // Sonra ogrenci tablosundan sil
                    return db.query(`DELETE FROM ogrenci WHERE TC_NO = '${actual_TC_NO}'`, { transaction: transaction });
                })
                .then(() => {
                    // İşlemi onayla
                    return transaction.commit();
                ...
                .catch((error) => {
                    // Hata olursa işlemi geri al
                    transaction.rollback();
                    console.error(`/api/ogrenci/aktifOgrenciSil TC_NO='${actual_TC_NO}'`, error);
                    res.status(500).json({ success: false, error: "Bir hata oluştu." });
                });
        });
    } else {
...
//api/malzeme/malzemeEkle
app.get("/api/malzeme/malzemeEkle", (req, res) => {
    const { MALZEME_ADI, STOK, MALZEME_BIRIM } = req.query;

    // Önce öğrencinin var olup olmadığını kontrol et
    db.query(`SELECT * FROM malzeme WHERE MALZEME_ADI = '${MALZEME_ADI}'`)
        .then(ogrenciResults => {
			const res = ogrenciResults.filter((malzeme) => malzeme.MALZEME_ADI === MALZEME_ADI);
            if (res.length > 0) {
                return res.status(404).json({ success: false, message: "Malzeme zaten var." });
            }
            // Öğrenci varsa, müsaitlik bilgisini ekle
            return db.query(`INSERT INTO malzeme (MALZEME_ADI, STOK, MALZEME_BIRIM) VALUES ('${MALZEME_ADI}', '${STOK}', '${MALZEME_BIRIM}')`);
        })
        .then(insertResults => {
            res.json({ success: true, message: "Malzeme bilgisi başariyla eklendi.", insertId: insertResults.insertId });
        })
        .catch(error => {
            res.status(500).json({ success: false, error: "Bir hata oluştu." });
        });
...
app.get("/api/malzeme/malzemeGetir", (req, res) => {
	db.query(
		`SELECT *
		FROM malzeme`
	)
